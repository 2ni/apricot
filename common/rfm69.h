...
class RFM69 {
  public:
    ...
    RFM69();
    uint8_t  init(uint16_t freq, uint32_t node_id, uint8_t network_id);
    uint8_t  init(uint32_t node_id, uint8_t network_id);
    ...
    void     send_frame(uint32_t to, const void* buffer, uint8_t size, uint8_t request_ack, uint8_t send_ack);
    void     sleep();
